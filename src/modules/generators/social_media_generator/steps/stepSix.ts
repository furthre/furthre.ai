...
export const initAdditionalInstructionsStep = (slider) => {
  const form = new WFFormComponent("#socialMediaGenerator");
  ...
  submitButton.on("click", (event) => {
    ...
    const updatedFormData = {
      ...
      additional_instructions: instructions, // Update additional instructions (allowed to be empty)
    };
      ...
      postFormData.onData((response: ApiResponse) => {
        if (response.content) {
          ...
          if (response.content.facebook_content) {
            sections.push(`<h1>Facebook</h1>${response.content.facebook_content}`);
          }
          if (response.content.youtube_content) {
            sections.push(`<h1>YouTube</h1>${response.content.youtube_content}`);
          }
          if (response.content.tiktok_content) {
            sections.push(`<h1>TikTok</h1>${response.content.tiktok_content}`);
          }
          ...
          if (response.content.linkedin_content) {
            sections.push(`<h1>LinkedIn</h1>${response.content.linkedin_content}`);
          }
